...
hf_mntp_model = 'jealk/llm2vec-scandi-mntp-v2'
hf_simcse_model = 'jealk/TTC-L2V-supervised-2'
hf_simcse_revision = ""

# Model name to save in SEB and local pkl {original model name}-{model type [instruct, llm2vec]}-{ft type}-{ft dataset}-{ft steps}
seb_model_name = 'TTC-L2V-supervised-2'
...
def task_to_instruction(task: Task) -> str:
    ...
    if task.task_type in ["Retrieval"]:
        task_name_to_instruct: dict[str, str] = {
            ...
            "SNL Retrieval": "Given a lexicon headline in Norsegian, retrieve its article",
...
def create_my_model() -> seb.SebModel:
    ...
    return seb.SebModel(
        encoder=SEB_L2V(l2v, max_length=256, max_batch_size=256),
        meta=meta,
    )
